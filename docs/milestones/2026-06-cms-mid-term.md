...
| 順 | 項目 | Issue | PR | 状態 |
| --- | --- | --- | --- | --- |
| 1 | Admin UI リデザイン＋レスポンシブ | #127 | #128 | ✅ |
| 2 | Full-text search API | #129 | #131 | ✅ |
| 3 | Export API (CSV/JSON) | #130 | #132 | ✅ |
| 4 | File field + upload（非画像） | #133 | #134 | ✅ |
| 5 | Webhooks | #135 | #136 | ✅ |
| 6 | Scheduled publish | #137 | #138 | ✅ |
| 7 | Draft preview tokens | #139 | #141 | ✅ |
| 8 | Public cache / ETag | #142 | #143 | ✅ |

### M4 — 機能拡充・使い勝手改善 ✅ 完了（2026-05-26）

管理画面 UX・多言語コンテンツ・ピン留め・デフォルトコンテンツタイプ・パーマリンク等。CHANGELOG 参照。

### M5 — ユーザー管理・メディアライブラリ ✅ 完了（2026-05-27）

User invite flow・パスワードリセット・メディアライブラリ管理 UI 等。CHANGELOG 参照。

### M6 — コメント機能 ✅ 完了（2026-05-27）

コメント投稿・承認モデレーション API・公開 UI・管理画面。CHANGELOG 参照。

### M7 — マルチテナント基盤・スーパー管理画面 ✅ 完了（2026-05-27）

`organizations` テーブル・CRUD API・`OrgResolverMiddleware`・全テーブルへの `organization_id` 追加・superadmin role・スーパー管理画面。CHANGELOG 参照。

### M8 — マルチテナント運用機能 ✅ 完了（2026-05-27）

テナント解決モード設定 UI（single/subdomain/path）・既存データ組織割当 UI・JSON 完全エクスポート／インポート。CHANGELOG 参照。

### M9 — マルチテナント完成（1ユーザー=1組織） ✅ 完了（2026-07-01）

`users` テーブルに `organization_id` / `org_role` 追加。`organization_users` テーブル廃棄。JWT に `org_id` 埋め込み。`CapabilityMiddleware` で組織スコープ強制。`organizations` に `external_id` 追加（外部連携用）。CHANGELOG 参照。

### M10 — マルチテナント包括テストスイート ✅ 完了（2026-07-01）

4 レベルのテストを実施: PHPUnit 545 tests（単体）/ Playwright 67 tests（1機能・ショートシナリオ・完全シナリオ）。CapabilityMiddleware org スコープ・LoginUseCase org_id 出力・ListUsersUseCase org フィルタ・CreateUserUseCase org 割当・InviteUser org 伝播・クロス org 分離・ロールアクセスマトリクスを網羅。CHANGELOG 参照。

### E2E 包括テスト拡充 ✅ 完了（2026-05-27）

Admin SPA の E2E テストを 67 → 157 件に拡充。UI 状態・アクセシビリティ・エラー回復・CRUD フロー・役割アクセスマトリクス・全管理ページ（Comments/Navigation/Webhooks/Media/Settings/Dashboard）・繰り返し安定性・認証ライフサイクルを網羅（Issue #260, PR #259）。
