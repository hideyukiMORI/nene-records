...
詳細は private `nene-origin/internal-docs/records/todo/current.md` のマイルストーン別セクション、および `CHANGELOG.md` を参照。
...
詳細は private `nene-origin/internal-docs/records/todo/current.md` を正本とする。

## Next Phase Candidates

Issue 化してから着手。詳細: private `nene-origin/internal-docs/records/todo/current.md` § 次フェーズ候補。
