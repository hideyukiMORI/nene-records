...
1. **前夜ピボット**: DNS 切替→**同一サーバ内の公開フォルダ切替**方式へ（施主決定）。records の実測回答=「ホスト焼き込みは .env `BASE_DOMAIN`＋`system_config.tenant_base_domain` の2箇所のみ・出力系は全てリクエストホスト由来・export 内焼き込み0」→ 切替は2行で済むと確定。
2. **前半デプロイ**: v0.5.3 zip 転送（sha256一致）→ 本番 docroot へ展開・**install/ を .htaccess で一時封鎖**（公開ホストの wizard 露出対策）・stg noindex（robots 差替方式）・CLI precheck READY（PHP 8.4.23・全拡張）。
...
5. **カットオーバー**: ホスト切替2行→施主がホスティング管理画面で apex+www の公開フォルダ切替→**伝播ゼロで着地**。
...
9. 旧サイトは非公開サブドメインへ退避・保全（アクセス制限・noindex・別DB・hub 施工）。SaaS 側 favicon の兄弟テナント漏れも撤去済み。
...
- ★運用学: カットオーバー前の二重管理は **permalink JOIN**（import で entity_id 振り直し・/company/ip=SaaS 1147→移設先 11 で実証）＋ SET NAMES＋live 残存 grep の3点セット。
...
- DB 内容変更: 20件超（全てバックアップ `pre_*_20260718` 付き・SaaS/移設先 両系）
...
- #949 の実ホスティング再現（chown で権限故障を作る）も次回リハ項目。
