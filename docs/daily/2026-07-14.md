...
- **実装**：現行インナーページのデザイン（`render.py` 系）で作成し company/products と統一（内側の新デザインは ClaudeDesign 待ちの方針＝勝手なリスキンはしない）。ビルダは内部作業レイヤーに保全。
...
> 横断コンテキスト（詳細は内部レイヤーの記録）：本日は vault（#198/199）・deal（#123/124）の red-team 診断結果を統合リナ側で取り込み（records の Critical/Medium 同型は両者に**不在**）、AYANE エビデンス（marketing repo の evidence doc §3.5）を「全 AYANE 関連製品 診断済み・EXPOSED 0」へ更新。
...
### 追記: #853（meta_title ラベル優先・PR #854）＋第30回デプロイ
施主レビュー第2弾「導出抜粋だと全行同じヘッダー文字列で区別できない」→ ラベル優先順位を
