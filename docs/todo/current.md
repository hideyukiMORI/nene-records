...
> repo-status 監査で見つかった文書齟齬3件を統合リナ協議のうえ解消（#899/#900・統合リナレビュー＆マージ）。
> 日報置き場 `docs/daily/` を CLAUDE.md に規範化（#901/#902・フリート裁定=リポごとの慣習が正）。
> **#896 は着手時調査で「実は 07-12 に media アップロード済み」と判明**し、contact 側実測 URL との
> 突合一致で受け入れ条件全達クローズ（本番無変更・#903/#904 で同期）→ **AYANE launch の records 側
> ブロッカーは 0**（残は contact 開通のみ・別リポ・確定 URL 3本連携済み）。
> 施主が本番で「読み込み中…」を目撃 → **#894 の棚卸し漏れ2箇所**（レコード本文=`PublicRecordDetailView`・
> 関連フィールド=`PublicRelationFieldDisplay`）と特定し、B群作法のスケルトンに置換（#905/#906・
> `public.record.loading` を全6ロケールから削除・「文字を出さない」を pin するテスト2本）。
> **第37回本番デプロイ（施主GO）**: migration 無し・フロントのみ。検証 = app healthy・apex/aozora/ayane 200・
> www→apex 301・エラーログ無し・**dist 直接 grep で旧キー消滅＋`sk-article`/`sk-line` 焼き込みを確認**
> （`public-site.css` は `PublicSiteShell` チャンクに分割されるため、エントリ CSS のハッシュ不変は正常）。
> 詳細 = `docs/daily/2026-07-16.md` 追補。

## 2026-07-16 午前: 公開一覧ラベル parity 修正＋読み込み中デザイン＋AYANE 残ブロッカー消化（**本番反映済み**）
