...
### 🔴 #896 — `/downloads/*.pdf` が一度も配信されていない（**contact 開通の前提**）
...
**AYANE 公開 = 07-19（残り3日）。残ブロッカーは contact フォーム開通（別リポ）と、その前提の #896。**
...
- **contact フォーム開通**（別リポ nene-contact）＝ **最後のブロッカー**
- **#896** `/downloads/*.pdf` が本番で一度も配信されていない（Apache Alias 欠落・#705 と同型）。
  ★**contact 開通の前提**（自動返信メールが 404 を3本張る）。方針 = **A: media にアップロード**（施主判断 07-16）
