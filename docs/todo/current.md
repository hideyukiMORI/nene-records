...
**M3 — P1 完了・P2 完了・P3 進行中** — 予約公開（#138 レビュー中）・Draft プレビュートークン（#141 レビュー中）

226 tests / PHPStan Max / CS-Fixer / OpenAPI / MCP バリデーション — 全グリーン。
...
| #137 | #138 | feat: 予約公開（scheduled_at + 自動ステータス遷移） |
| #139 | #141 | feat: Draft プレビュートークン（非公開レコードの外部プレビュー URL） |

## Up Next — M3 P3（残り）

| 優先 | 項目 | 説明 |
| --- | --- | --- |
| P3 | Public cache / ETag | bootstrap・settings の Cache-Control 整備 |
...
```bash
composer check                    # 226 tests + PHPStan + CS-Fixer + OpenAPI + MCP
