...
5. 🟡 管理 UI：テーマピッカーに runtime テーマを**合成表示**（built-in＋runtime、`swatchFromManifest` でカード）、active 解決を runtime キー対応、採用は `active_theme` 書込みで完了（#423 Phase E）。**残**: 一覧での編集/削除 UI。
